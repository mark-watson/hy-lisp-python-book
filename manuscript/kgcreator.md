...
1. **Expanding Entity Types**: In [kgcreator.hy](file:///Users/markwatson/GITHUB/hy-lisp-python-book/source_code_for_examples/kgcreator/kgcreator.hy), only a subset of spaCy's entity types (`NORP`, `ORG`, `PRODUCT`, `GPE`, `PERSON`, and `LOC`) are converted to RDF. Modify both `e2umap` and the filtering logic in `process-file` to support additional entity types such as `DATE` (mapped to `https://schema.org/Date`), `MONEY` (mapped to `https://schema.org/MonetaryAmount`), and `EVENT` (mapped to `https://schema.org/Event`).
2. **Dynamic DBpedia URI Resolution**: In [kgcreator_uri.hy](file:///Users/markwatson/GITHUB/hy-lisp-python-book/source_code_for_examples/kgcreator/kgcreator_uri.hy), the `v2umap` dictionary maps string entity names to DBpedia URIs statically. Implement a dynamic mapping mechanism. For instance, format the entity name by replacing spaces with underscores and appending it to `http://dbpedia.org/page/` (or query the DBpedia Lookup Web API directly) to resolve URIs programmatically.
3. **RDF Triple Deduplication**: If an entity is mentioned multiple times within the same document, the current scripts write duplicate RDF statements to the output file. Modify `process-file` or `data2Rdf` to collect entities in a set, preventing duplicate subject-predicate-object triples from being written for the same subject URI.
